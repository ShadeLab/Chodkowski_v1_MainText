...
filt <- read.csv("MassSpec/releaseAnalysis/MS/initialFiles/MZmineNonPolarPoswExConForQC.csv",check.names=F)
...
write.csv(exoMetabMonoCulture,"MassSpec/releaseAnalysis/MS/initialFiles/MZmineNonPolarPosMonoculture.csv",row.names=FALSE)
exoMetabolomeMonoCulture <- read.csv("MassSpec/releaseAnalysis/MS/initialFiles/MZmineNonPolarPosMonoculture.csv",check.names=F)
#Order columns
exoMetabolomeMonoCultureOrdered <- exoMetabolomeMonoCulture[ , order(names(exoMetabolomeMonoCulture))]
write.csv(exoMetabolomeMonoCultureOrdered,"MassSpec/releaseAnalysis/MS/initialFiles/MZmineNonPolarPosMonocultureOrdered.csv",row.names=FALSE)
...
groupedMonoSum$conditions <- rep(c("Bt","Cv","Ps"),each=6,18669)
#Add time
groupedMonoSum$time <- rep(c(12.5,25,30,35,40,45),56007) #This number is the number above * 3 (# of conditions)
...
BtMinMaxIDs <- unique(BtmonoFC$ID[BtmonoFC$FC>3]) #852
CvMinMaxIDs <- unique(CvmonoFC$ID[CvmonoFC$FC>3]) #895
PsMinMaxIDs <- unique(PsmonoFC$ID[PsmonoFC$FC>3]) #894
...
BtFinalIDs <- groupRSDSumBtMAX$ID[groupRSDSumBtMAX$RSD<0.2] #383
CvFinalIDs <- groupRSDSumCvMAX$ID[groupRSDSumCvMAX$RSD<0.2] #551
PsFinalIDs <- groupRSDSumPsMAX$ID[groupRSDSumPsMAX$RSD<0.2] #623
...
groupRSDSumCv0s[which(groupRSDSumCv0s$ID %in% CvFinalIDs),] #One feature in 25hr but also at initial TP, too. This is okay
...
saveRDS(BtFinalIDs, "MassSpec/releaseAnalysis/MS/outputFiles/BtFinalIDsNonPolarPos.rds")
saveRDS(CvFinalIDs, "MassSpec/releaseAnalysis/MS/outputFiles/CvFinalIDsNonPolarPos.rds")
saveRDS(PsFinalIDs, "MassSpec/releaseAnalysis/MS/outputFiles/PsFinalIDsNonPolarPos.rds")
...
finalFeats <- list(Bt=BtFinalIDsNonPolarPos,Cv=CvFinalIDsNonPolarPos,Ps=PsFinalIDsNonPolarPos)
finalFeatsV <- lapply(finalFeats, function(x) x[!is.na(x)])
library(VennDiagram)

#Supplementary Figure 1
venn.diagram(finalFeatsV,filename="Figures/SFig1/MonoOverlappingFeaturesNonPolarPos.tiff")

#Venn output shows
#327 unique features for Bt
#414 unique features for Cv
#505 unique features for Ps
...
exoMetabolomeReleased <- rbind(exoMetabolomeReleased,exoMetabolome[exoMetabolome$ID=="33",])
...
#1) MZmineNonPolarPosallReleasedMetabolitesIndividualSamples_readyForMetaboAnalyst_wTime.csv for HeatMap analysis.
...
#2) MZmineNonPolarPosallReleasedMetabolitesIndividualSamples_readyForMetaboAnalyst.csv for PCA analysis
    #Data was normalized in metaboanalyst and normalized data was exported to NonPolarPos_allReleased_IndSamples_MetaboanalystNormalization.csv
      #This was then uploaded in R to perform PCA in vegan, generating Figure 2.

#MZmineNonPolarPosallReleasedMetabolitesIndividualSamples.csv manually edited file was uploade to MetaboAnalyst
...
MSMSmatchesNist17 <- read.csv("MassSpec/releaseAnalysis/MSMS/initialFiles/MZmineNonPolarPosMSMSnist17Matches.csv",check.names=F)
MSMSmatchesMONA <- read.csv("MassSpec/releaseAnalysis/MSMS/initialFiles/NonPolarPosMSMS_MONAspectraMatches.csv",check.names=F)
...
#Nist17 redundants
length(nist17hits$"cluster index")   #21
length(unique(nist17hits$"cluster index"))  #21
length(nist17hits$LibraryID)   #21
length(unique(nist17hits$LibraryID))  #20
...
#MONA redundants
length(MONAhits$rowID) #23
length(unique(MONAhits$rowID)) #23
length(MONAhits$LibraryID) #23
length(unique(MONAhits$LibraryID)) #21
#obtain redundant features
...
#Add internal standard- 2­Amino­3­bromo­5­methylbenzoic acid, ID 33
filtFinalMSMS <- rbind(filtFinalMSMS,exoMetabolomeMonoCulture[exoMetabolomeMonoCulture$ID=="33",])
#Re-order columns
filtFinalMSMSO <- filtFinalMSMS[ , order(names(filtFinalMSMS))]
#Write csv for individual samples
write.csv(filtFinalMSMSO,"MassSpec/releaseAnalysis/MSMS/outputFiles/MZmineNonPolarPosReleasedMetaboliteswIDS_IndividualSamples.csv")

#Write compounds to CSV
write.csv(identifiedFinal,"MassSpec/releaseAnalysis/MSMS/outputFiles/NonPolarPosMSMScompounds.csv")
...
#For example, remove Tetrahydrozoline HCl, Quinaldic acid, Neoabietic acid, NCGC00381398-01_C17H26O4_6-[(2E,4E)-4,6-Dimethyl-2,4-octadienoyl]-2,3-dihydroxy-2-methylcyclohexanone
  #N-(2,4-dimethylphenyl)formamide, methyl 2-ethyl-4-[(3R,4R,5S)-5-hydroxy-4,5-dimethyl-2-oxooxolan-3-yl]-2-methyl-3-oxobutanoate,Diphenylamine,D-Gluconic acid, .delta.-lactone
  #Dehydroabietamide,Coumarin, Chamissonolide,Cer-NP t32:0, Amantadine,6-[2-(1,3-benzodioxol-5-yl)ethyl]-4-methoxypyran-2-one,  3,15-dibenzyl-1,4,12-trimethyl-1,4,7,10,13-pentazacyclopentadecane-2,5,8,11,14-pentone
