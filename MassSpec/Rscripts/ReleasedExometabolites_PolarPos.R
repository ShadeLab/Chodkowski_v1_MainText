...
filt <- read.csv("MassSpec/releaseAnalysis/MS/initialFiles/MZminePolarPoswExConForQC.csv",check.names=F)
...
write.csv(exoMetabMonoCulture,"MassSpec/releaseAnalysis/MS/initialFiles/MZminePolarPosMonoculture.csv",row.names=FALSE)
exoMetabolomeMonoCulture <- read.csv("MassSpec/releaseAnalysis/MS/initialFiles/MZminePolarPosMonoculture.csv",check.names=F)
#Order columns
exoMetabolomeMonoCultureOrdered <- exoMetabolomeMonoCulture[ , order(names(exoMetabolomeMonoCulture))]
write.csv(exoMetabolomeMonoCultureOrdered,"MassSpec/releaseAnalysis/MS/initialFiles/MZminePolarPosMonocultureOrdered.csv",row.names=FALSE)
...
groupedMonoSum$conditions <- rep(c("Bt","Cv","Ps"),each=6,36250)
#Add time
groupedMonoSum$time <- rep(c(12.5,25,30,35,40,45),108750) #This number is the number above * 3 (# of conditions)
...
BtMinMaxIDs <- unique(BtmonoFC$ID[BtmonoFC$FC>3]) #3502
CvMinMaxIDs <- unique(CvmonoFC$ID[CvmonoFC$FC>3]) #2704
PsMinMaxIDs <- unique(PsmonoFC$ID[PsmonoFC$FC>3]) #3236
...
BtFinalIDs <- groupRSDSumBtMAX$ID[groupRSDSumBtMAX$RSD<0.2] #2446
CvFinalIDs <- groupRSDSumCvMAX$ID[groupRSDSumCvMAX$RSD<0.2] #1692
PsFinalIDs <- groupRSDSumPsMAX$ID[groupRSDSumPsMAX$RSD<0.2] #1917

#Confirm filtered IDs that also had a RSD value of 0 was the RSD value at the initial timepoint
groupRSDSumBT0s[which(groupRSDSumBT0s$ID %in% BtFinalIDs),] #Looks good
groupRSDSumCv0s[which(groupRSDSumCv0s$ID %in% CvFinalIDs),] #Two features in 25hr but also at initial TP, too. This is okay
groupRSDSumPs0s[which(groupRSDSumPs0s$ID %in% PsFinalIDs),] #Looks good

#Do not need to remove any additional IDs

#Save IDs for each isolate that have been determined to be released
saveRDS(BtFinalIDs, "MassSpec/releaseAnalysis/MS/outputFiles/BtFinalIDsPolarPos.rds")
saveRDS(CvFinalIDs, "MassSpec/releaseAnalysis/MS/outputFiles/CvFinalIDsPolarPos.rds")
saveRDS(PsFinalIDs, "MassSpec/releaseAnalysis/MS/outputFiles/PsFinalIDsPolarPos.rds")
...
finalFeats <- list(Bt=BtFinalIDsPolarPos,Cv=CvFinalIDsPolarPos,Ps=PsFinalIDsPolarPos)
finalFeatsV <- lapply(finalFeats, function(x) x[!is.na(x)])
library(VennDiagram)

#Supplementary Figure 1
venn.diagram(finalFeatsV,filename="Figures/SFig1/MonoOverlappingFeatures_PolarPos.tiff")

#Venn output shows
#1955 unique features for Bt
#985 unique features for Cv
#1266 unique features for Ps
...
#Add internal standard- 13C-15N-Alanine, ID 107
exoMetabolomeReleased <- rbind(exoMetabolomeReleased,exoMetabolome[exoMetabolome$ID=="107",])
...
saveRDS(NA_Index,"MassSpec/releaseAnalysis/MS/outputFiles/PolarPos_NA_Index.rds")
...
#1) MZminePolarPosallReleasedMetabolitesIndividualSamples_readyForMetaboAnalyst_wTime.csv for HeatMap analysis.
...
#2) MZminePolarPosallReleasedMetabolitesIndividualSamples_readyForMetaboAnalyst.csv for PCA analysis
    #Data was normalized in metaboanalyst and normalized data was exported to PolarPos_allReleased_IndSamples_MetaboanalystNormalization.csv
...
#Upload JGI library
library(dplyr)
JGIrefLib <- read.csv("MassSpec/releaseAnalysis/MSMS/initialFiles/JGI_refLibrary_PolarPosUpdated.csv",check.names=F)
...
table(table(allJGI$Label)) #35 0's, 100 1's- 35 JGI exometabolites not detected
...
Cvmetabs <- allJGIreleased[which(allJGIreleased$ID %in% CvFinalIDsPolarPos),]

#Extract released Ps exometabolites from JGI
Psmetabs <- allJGIreleased[which(allJGIreleased$ID %in% PsFinalIDsPolarPos),]
...
MSMSmatchesNist17 <- read.csv("MassSpec/releaseAnalysis/MSMS/initialFiles/MZminePolarPosMSMSnist17Matches.csv",check.names=F)
MSMSmatchesMONA <- read.csv("MassSpec/releaseAnalysis/MSMS/initialFiles/PolarPosMSMS_MONAspectraMatches.csv",check.names=F)
...
#JGI redundants
length(allJGIreleased$ID) #29
length(unique(allJGIreleased$ID)) #26
length(allJGIreleased$Label) #29
length(unique(allJGIreleased$Label)) #29
#Note, homoserine, threonine, L-Allothreonine are isomers (32866). Leucine and norleucine are isomers (17722).
allJGIreleased$ID[which(duplicated(allJGIreleased$ID))]

#Nist17 redundants
length(nist17hits$"cluster index")   #48
length(unique(nist17hits$"cluster index"))  #48
length(nist17hits$LibraryID)   #48
length(unique(nist17hits$LibraryID))  #38
...
#MONA redundants
length(MONAhits$rowID) #51
length(unique(MONAhits$rowID)) #51
length(MONAhits$LibraryID) #51
length(unique(MONAhits$LibraryID)) #41
#obtain redundant features
...
nist17hitsSub <- nist17hits[,c(11,3)]
...
identifiedFinal <- rbind(allJGIreleased,nist17hitsSub,MONAhitsSub)
...
#Add internal standard- 13C-15N-Alanine, ID 107
filtFinalMSMS <- rbind(filtFinalMSMS,exoMetabolomeMonoCulture[exoMetabolomeMonoCulture$ID=="107",])
#Re-order columns
filtFinalMSMSO <- filtFinalMSMS[ , order(names(filtFinalMSMS))]
#Write csv for individual samples
write.csv(filtFinalMSMSO,"MassSpec/releaseAnalysis/MSMS/outputFiles/MZminePolarPosReleasedMetaboliteswIDS_IndividualSamples.csv")

#Write compounds to CSV
write.csv(identifiedFinal,"MassSpec/releaseAnalysis/MSMS/outputFiles/PolarPosMSMScompounds.csv")

######################################################Manually edit file######################################################

#The file: MZminePolarPosReleasedMetaboliteswIDS_IndividualSamples.csv was manually edited

#First, align and copy over identified metabolites and source from PolarPosMSMScompounds onto the MZminePolarPosReleasedMetaboliteswIDS_IndividualSamples CSV file
...
#For example, remove Gabapentin, Pargyline, Caffeate, (2E,4E)-N-(2-methylpropyl)dodeca-2,4-dienamide, (3R,7R,8R,8aS)-3,4'-dihydroxy-7'-(2-hydroxyethyl)-4,4,7,8a-tetramethylspiro[2,3,4a,5,6,7-hexahydro-1H-naphthalene-8,2'-3,8-dihydrofuro[2,3-e]isoindole]-6'-one
  #1,4-dihydroxy-6,6,9a-trimethyl-4,5,5a,7,8,9-hexahydro-1H-benzo[e][2]benzofuran-3-one, 2-Hexenal,3,5-Lutidine, 5-Amino-1-.beta.-D-ribofuranosyl-1H-imidazole-4-carboxamide
  #8-acetamido-2-methyl-7-oxononanoic acid, Cyclohexylamine, Isopentenyladenine, Lupinine, Methyprylon, Miglitol,2,6-diisopropylaniline
  #MYOSMINE, N,N-Diethyl-2-aminoethanol,NCGC00385817-01!(2E,4E)-N-(2-methylpropyl)deca-2,4-dienamide,Quinaldic acid, Threitol, 1-Acetylimidazole
...
#####The image file was used to generate Figure 2
...
JGIPP_IDs <- read.csv("MassSpec/releaseAnalysis/MSMS/outputFiles/PolarPos_JGI_MSL1_IDs.csv",sep=",",header=TRUE,check.names=FALSE)
exoMetabolomeMonoCulture <- read.csv("MassSpec/releaseAnalysis/MS/initialFiles/MZminePolarPosMonoculture.csv",check.names=F)

#Avg BioReps
library(reshape2)
JGIPP_IDsMelt <- melt(JGIPP_IDs,id.vars=c("ID","RT","MZ"))

#Change NA values and 0s to low value
JGIPP_IDsMelt[,5][JGIPP_IDsMelt[,5]<2] <- 2

#We need to split names so we can average conditions within a time point

JGIsplit <- data.frame(do.call('rbind', strsplit(as.character(JGIPP_IDsMelt$variable),'-',fixed=TRUE)))
#Place this into melted object
JGIPP_IDsMelt$groupTime <- JGIsplit$X1

library(dplyr)
detach("package:plyr", unload=TRUE)
JGIPP_IDsMelt_grouped <- JGIPP_IDsMelt %>% group_by(ID,RT,MZ,groupTime)
#Get mean
JGIPP_IDsMelt_groupedAvg <- summarise(JGIPP_IDsMelt_grouped, mean=mean(value))

#Convert back to DF
library(reshape2)
JGIPP_Avg.df <- dcast(JGIPP_IDsMelt_groupedAvg, ID + MZ + RT ~ groupTime)

#Get top 5 most abundant exometabolites for B.thailandensis at 45 hr
Bt_exoMetab <- JGIPP_Avg.df[,c(1:3,9)]
...
Cv_exoMetab <- JGIPP_Avg.df[,c(1:3,15)]
...
Ps_exoMetab <- JGIPP_Avg.df[,c(1:3,21)]
...
Bt_PP_topFiveMSL1 <- allJGI$Label[which(allJGI$ID %in% Bt_mostAbunTF)]
Cv_PP_topFiveMSL1 <- allJGI$Label[which(allJGI$ID %in% Cv_mostAbunTF)]
Ps_PP_topFiveMSL1 <- allJGI$Label[which(allJGI$ID %in% Ps_mostAbunTF)]
...
BtmostAbunAndReleased <- allJGI$Label[which(allJGI$ID %in% BtFinalIDsPolarPos[which(BtFinalIDsPolarPos %in% Bt_mostAbunTF)])]
CvmostAbunAndReleased <- allJGI$Label[which(allJGI$ID %in% CvFinalIDsPolarPos[which(CvFinalIDsPolarPos %in% Cv_mostAbunTF)])]
PsmostAbunAndReleased <- allJGI$Label[which(allJGI$ID %in% PsFinalIDsPolarPos[which(PsFinalIDsPolarPos %in% Ps_mostAbunTF)])]
